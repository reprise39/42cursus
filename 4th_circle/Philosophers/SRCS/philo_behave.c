...
bool take_forks(t_simulation *sim, t_philosopher *philosopher)
{
	...
	printf("%ld_in_ms %d has taken (%d) fork\n", cal_mili_sec_time (&sim->start_time, &current_time),philosopher->id , first_fork);
	if(is_dead(sim))
		return (false);
	pthread_mutex_lock(&sim->thread_manage.forks_mutex[last_fork]);
	gettimeofday(&current_time, NULL);
	printf("%ld_in_ms %d has taken (%d) fork\n", cal_mili_sec_time (&sim->start_time, &current_time), philosopher->id ,last_fork);
	if(is_dead(sim))
		return (false);
	return (true);
}
...
bool eating(t_simulation *sim, t_philosopher *philosopher)
{
	...
	if (sim->condition.optional_eat_times_flag == 1 && philosopher->num_of_eat_times == sim->condition.num_of_eat_times)
	{
		printf("Philosopher %d has finished eating %d times\n ~fin~ \n", philosopher->id, philosopher->num_of_eat_times);
		pthread_exit(NULL); // Exit the thread if the philosopher has eaten enough
	}
	return (true);
}
