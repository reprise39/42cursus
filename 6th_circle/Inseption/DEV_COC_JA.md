...
2. **設定**:
   `secrets` ディレクトリ内に `.txt` ファイルを作成し、タイトルのパスワードを設定します。

> **補足:** クローン時に存在する.txt.exampleから.exampleを削除して中身を記述すれば良い形に準備されています。パスワードは公開しないでください。以下の方法、またはそれ相当の強度のパスワード作成をおすすめします。`pwgen 16 1`
...
> **注意:** make clean , make fcleanではこれらも削除されます。注意してください。